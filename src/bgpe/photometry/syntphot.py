...
from bgpe.util.constants import c_AngSec as c # Speed of light in anstroms per second
...
class photoconv(object):
    ...
    def spec2filter(self, filter, obs_spec, model_spec=None, badpxl_tolerance = 0.5, dlambda_eff = None):
        ...
        if model_spec != None:
            ...
            if len(model_cut) == 0:
                self.log.warning('No enough MODEL datapoints eval synthetic photometry on this filter. Model will NOT be considered.')
                model_spec = None
            elif np.any(model_cut['wl'] != obs_cut['wl']):
                self.log.warning('Model is not sampled the same way as the observed spectrum. Interpolating...')
        ...
        if(np.any(obs_cut['wl'] != filter['wl'])):
            self.log.warning('Filter is not sampled the same way as the observed spectrum. Interpolating...')
            wl_ = obs_cut['wl']
        ...
        if(dlambda_eff == None):
            ...
            self.log.warning('dlambda_eff not defined, using %3.2f', dlambda_eff)
        self.log.debug('dlambda_eff = %3.2f', dlambda_eff)
        ...
        if('flag' in obs_cut.dtype.names and 'error' in obs_cut.dtype.names): # First check if there is error AND flag.
            ...
            bad     = np.invert(good)
            n_bad   = np.sum(bad)
        self.log.debug( 'N_points: %d, N_bad: %d' % (n, n_bad) )
        
        
        if(n_bad > 0): #If we have problems we have to deal with them... ;)
            ...
            if(p_bad > badpxl_tolerance): #If we have # of bad pixels greater than 50%, then make filter flux and error equal to inf.
                self.log.debug('# of bad pixels > badpxl_tolerance. m = inf and m_err = inf')
                m_ab = np.inf
                e_ab = np.inf
                return m_ab, e_ab
            else:
    ...
    def spec2filterset(self, filterset, obs_spec, model_spec, badpxl_tolerance = 0.5, dlambda_eff = None):
        ...
        for i_filter in range(len(filter_ids)):
            ...
            mags[i_filter]['m_ab'], mags[i_filter]['e_ab'] = self.spec2filter(filter, obs_spec, model_spec, badpxl_tolerance = badpxl_tolerance, dlambda_eff = dlambda_eff)
            self.log.debug('Magnitude to filter %s: %3.2f, error: %3.2f' % (filter_ids[i_filter], mags[i_filter]['m_ab'], mags[i_filter]['e_ab']) )
        return mags
    ...
    def fromStarlight(self, filterset, arq_in, arq_syn, starlight_version='starlightv4', badpxl_tolerance=0.5, dlambda_eff = None):
        ...
        return self.spec2filterset(filterset, obs_spec, model_spec, badpxl_tolerance = badpxl_tolerance, dlambda_eff = dlambda_eff)
    ...
    def fromSDSSfits(self, filterset, fits, badpxl_tolerance = 0.5):
        ...
        return self.spec2filterset(filterset, self.obs_spec, self.model_spec, badpxl_tolerance, dlambda_eff = 3.0)